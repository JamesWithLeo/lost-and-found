...
export default function ItemCard({
  ...
  return (
    <Link
      href={href}
      className={`${bg} grid ${view === "grid" ? "h-32 grid-cols-1" : "h-20 grid-cols-3 grid-rows-1"} w-full gap-2 rounded-lg border border-gray-200 p-4`}
    >
      ...
      >
        ...
        <h1 className={`text-sm ${anony.className}`}>
          {claimantCount} {i.type === "found" ? "Claimants" : "leads"}
        </h1>
        {view === "grid" ? (
          <h1
